...
`podman build -t mvn -f Dockerfile_mvn` (important, the name must be `mvn` otherwise you will need to change the other Dockerfile below)
...
`podman build -t gs -f Dockerfile_cache`
